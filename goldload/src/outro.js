...
export default class Outro extends Phaser.Scene {
    ...
    async create () {
        ...
        this.cameras.main.setBackgroundColor(0x0eb7b7)
        this.cloudLayer = this.add.layer();    
        await this.saveScore();
        //
        this.add.bitmapText(this.center_width,70, "pixelFont", "SCOREBOARD", 60).setOrigin(0.5).setDropShadow(0, 8, 0x222222, 0.9);
        this.restartText = this.add.bitmapText(this.center_width, 760, "pixelFont", "Click HERE to Restart", 20).setOrigin(0.5).setDropShadow(0, 8, 0x222222, 0.9);
    ...
    async saveScore () {
        this.currentId = 0;
        const hits = +this.registry.get("totalGolds");
        if (hits === 0) {
    ...
    showPrompt () {
        ...
        this.userName = "";
        this.userNameText = this.add.bitmapText(this.center_width, this.center_height - 150, "pixelFont", this.userName.padEnd(9, '-'), 50).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 8, 0x222222, 0.9);
    ...
    async saveScoreWithName () {
        ...
        this.currentId = await saveData(+this.registry.get("totalGolds"), this.userName)
        await this.loadScores();
    }
    ...
    async notBigger (score) {
        try {
            ...
            const ballBreakerScores = scores.filter(score => score.game === "GetRichOrDieTrying")
    ...
    async loadScores () {
        ...
        ballBreakerScores.splice(0, 10).forEach( (score, i) => {
            const text0 = this.add.bitmapText(this.center_width - 350, 170 + (i * 60), "pixelFont", `${i+1}`, 30).setOrigin(0.5).setDropShadow(0, 6, 0x222222, 0.9);
            const text1 = this.add.bitmapText(this.center_width - 150, 170 + (i * 60), "pixelFont", `${score.player.substring(0, 10).padEnd(11, ' ')}`, 30).setOrigin(0.5).setDropShadow(0, 6, 0x222222, 0.9);
            const text2 = this.add.bitmapText(this.center_width + 200, 170 + (i * 60), "pixelFont", `${String(score.score).padStart(10, '0')}`, 30).setOrigin(0.5).setDropShadow(0, 6, 0x222222, 0.9);
    ...
    addLetters () {
       ...
       // this.add.rectangle(250, 740, 500, 200, 0x4d4d4d).setOrigin(0.5);
        ...
        this.helpText = this.add.bitmapText(this.center_width, 630, "pixelFont", "", 30).setTint(0x4d4d4d).setOrigin(0.5)
      }
}
