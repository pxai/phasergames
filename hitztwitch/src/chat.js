...
export default class Chat {
    ...
    init () {
        ...
        this.client.on("message", (channel, tags, message, self) => {
            ...
            if (this.isValidWord(message)) {
                this.scene.guess(tags.username, message.trim().toLowerCase());
            }
        });
        ...
        this.client.on("chat", async (channel, user, message, self) => {
            ...
            if (this.isValidWord(message)) {
                this.scene.guess(username, message.trim().toLowerCase());
            }
        });
    }
    ...
    isValidWord (word) {
        return word.trim().split(" ").length === 1 && word.trim().length > 2;
    }
}
