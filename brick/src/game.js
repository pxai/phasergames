...
export default class Game extends Phaser.Scene {
    ...
    addControls () {
      this.input.on('pointerdown', function (mouse) {
        this.paintLine();
      }, this);
    }
    ...
    addMap () {
      console.log("Loading map: ", this.number)
    ...
    hitFloor (ball, platform) {
      ...
      if (platform.index === 1) {
        ...
        this.updateRecycled();
        this.tweens.add({
          targets: [this.recycledBrick, this.textRecycled],
          alpha: {from: 0.7, to: 1},
          scale: {from: 0.6, to: 1},
          duration: 30,
          repeat: 10
    ...
    showRecycled() {
      this.recycledBrick = this.add.sprite(86, 50, "brick0").setOrigin(0.5)
      this.textRecycled = this.add.bitmapText(128, 50, "mario", "x" + this.recycled, 20).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 4, 0x222222, 0.9);
    }

    showStage() {
      this.text1 = this.add.bitmapText(this.center_width, 50, "mario", "STAGE " + this.number, 20).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 4, 0x222222, 0.9);

      if (this.number === 0) {
        this.text2 = this.add.bitmapText(this.center_width, this.center_height, "mario", "CLICK TO ADD BLOCK", 25).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 4, 0x222222, 0.9);
        this.time.delayedCall(3000, () => this.text2.destroy());
      }
    }

    showHits() {
      this.textHits = this.add.bitmapText(this.width - 128, 50, "mario", "HITS: " + this.registry.get("hits"), 20).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 4, 0x222222, 0.9);
    }
    ...
    paintLine() {
      if (this.recycled < 1) return;
      this.playAudio("place");
      this.updateRecycled(-1)
    ...
    gotcha (ball, entrance) {
      this.playAudio("gotcha")
      this.basket.anims.play("basket", true);
      new StarBurst(this, entrance.x, entrance.y);
      this.ball.destroy();
      this.playAudio("marble")

      this.textYAY = this.add.bitmapText(this.center_width, this.center_height, "mario", "Clear!", 80).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 8, 0x222222, 0.9);
      this.tweens.add({
        targets: this.basket,
        duration: 100,
    ...
    finishScene () {
      if (this.number < 6) {
        ...
        this.text3 = this.add.bitmapText(this.center_width, this.center_height + 50, "mario", "HITS: " + this.registry.get("hits"), 45).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 8, 0x222222, 0.9);
        this.text4 = this.add.bitmapText(this.center_width, this.center_height - 50, "mario", "CONGRATULATIONS!", 35).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 5, 0x222222, 0.9);
    ...
    updateRecycled (amount = 1) {
      this.recycled += amount; 
      this.textRecycled.setText( "x" + this.recycled)
    }

    death() {
      this.cameras.main.shake(100);
      this.textBOO = this.add.bitmapText(this.center_width, this.center_height, "mario", "FAIL", 80).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 5, 0x222222, 0.9);
