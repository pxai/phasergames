...
export default class Outro extends Phaser.Scene {
    ...
    showPrompt () {
        ...
        this.userName = "";
        this.userNameText = this.add.bitmapText(this.center_width, this.center_height - 150, "pixelFont", this.userName.padEnd(9, '-'), 70).setTint(0xb95e00).setOrigin(0.5).setDropShadow(0, 8, 0x222222, 0.9);
    ...
    async saveScoreWithName () {
        ...
        this.currentId = await saveData(+this.registry.get("seconds"), this.userName)
        await this.loadScores();
    }
    ...
    async loadScores () {
        ...
        ballBreakerScores.splice(0, 10).forEach( (score, i) => {
            const text0 = this.add.bitmapText(this.center_width - 350, 170 + (i * 60), "pixelFont", `${i+1}`, 50).setOrigin(0.5).setDropShadow(0, 6, 0x222222, 0.9);
            const text1 = this.add.bitmapText(this.center_width - 150, 170 + (i * 60), "pixelFont", `${score.player.substring(0, 10).padEnd(11, ' ')}`, 50).setOrigin(0.5).setDropShadow(0, 6, 0x222222, 0.9);
            const text2 = this.add.bitmapText(this.center_width + 200, 170 + (i * 60), "pixelFont", `${String(score.score).padStart(10, '0')}`, 50).setOrigin(0.5).setDropShadow(0, 6, 0x222222, 0.9);
    ...
    addLetters () {
       ...
       // this.add.rectangle(250, 740, 500, 200, 0x4d4d4d).setOrigin(0.5);
        ...
        this.helpText = this.add.bitmapText(this.center_width, 630, "pixelFont", "", 50).setTint(0x4d4d4d).setOrigin(0.5)
      }
}
