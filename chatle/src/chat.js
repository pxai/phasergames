...
export default class Chat {
    ...
    init () {
        ...
        this.client.on("message", (channel, tags, message, self) => {
            console.log(`Message: ${tags.username} just ${message} valid: `, this.isValidWord(message));
            // if(self) return;
            ...
            console.log("Received chat: ", channel, tags.username, messageParts, this.scene);

            if (this.isValidWord(message)) {
                this.scene.guess(tags.username, message.slice(1).toLowerCase());
            }
        });
        ...
        this.client.on("chat", async (channel, user, message, self) => {
            ...
            const messageParts = message.toLowerCase().split(" ");
            console.log("Received chat: ", channel, user, messageParts, this.scene, this.isValidWord(message));
            const username = user["display-name"];

            if (this.isValidWord(message)) {
                this.scene.guess(username, message.slice(1).toLowerCase());
            }
        });
    }
    ...
    isValidWord (word) {
        return word.trim().startsWith("!") && word.trim().length === 6;
    }
}
